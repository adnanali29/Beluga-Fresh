...
export const ProductSVG: React.FC<ProductSVGProps> = ({ type }) => {
  ...
  if (type.startsWith('mix-')) {
    ...
    const mal = type === 'mix-avial' ? 'അവിയൽ മിക്സ്' : type === 'mix-sambar' ? 'സദ്യ സാമ്പാർ മിക്സ്' : type === 'mix-thooran' ? 'തോരൻ മിക്സ്' : type === 'mix-kalan' ? 'കാളൻ മിക്സ്' : 'ഓലൻ മിക്സ്';
    ...
    return (
      <svg viewBox="0 0 200 240" className="h-44 w-auto drop-shadow-md">
        ...
        <text x="100" y="93" textAnchor="middle" fill="#334155" fontSize="7" fontWeight="bold">{mal}</text>
  ...
  const emojiMap: Record<string, string> = {
    ...
    'produce-pineapple': '🍍',
    'produce-curryleaves': '🌿',
    'produce-bananaleaves': '🍃'
  };

  return (
    <div className="text-center py-4">
      <span className="text-5xl">{emojiMap[type] || '🌱'}</span>
